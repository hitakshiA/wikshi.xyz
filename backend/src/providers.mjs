  ...
  async execute(op,store,{signal}={}) {
    ...
    if(service==='video.meeting') {
      const agent=await this.bey('/agents','POST',{name:'Wikshi',avatar_id:this.env.BEY_AVATAR_ID,language:'en',max_session_length_minutes:input.maxSeconds/60,
        ...
        system_prompt:`Conduct a focused conversation comfortably within ${input.maxSeconds} seconds. This is a maximum, not a target: do not stretch a finished conversation. Identify as AI and obtain consent. No big introduction or rigid questionnaire. Ask related questions together, adapt to answers, respect refusals, and summarize only confirmed facts. Once all meeting objectives have been covered, or the guest is done or declines, give one short closing summary, thank them, and explicitly ask them to end the call using the meeting's end-call control. For example: "We've covered everything we needed. Thank you for your time. Please end the call when you're ready." Do not invent extra questions to fill the remaining time, repeatedly nag the guest, reopen the discussion, or claim you have disconnected the room. Never promise external actions or commitments. Mission: ${input.mission}\nEssential questions: ${input.questions.join(' / ')}`});
