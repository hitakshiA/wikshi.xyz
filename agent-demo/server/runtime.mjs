...
export function createRuntime(session,emit) {
  ...
  const tools=[
    tool('suggest_next_steps','Offer up to three natural next-step prompt buttons based on this conversation. This only suggests prompts; it never prepares, approves, pays, sends, or executes anything. Call near the end of a response when useful next steps remain, including after completion summaries. Do not offer actions already awaiting approval.',{type:'object',properties:{suggestions:{type:'array',maxItems:3,items:{type:'object',properties:{label:{type:'string',maxLength:60},prompt:{type:'string',maxLength:600}},required:['label','prompt'],additionalProperties:false}}},required:['suggestions'],additionalProperties:false},async({suggestions})=>{const items=normalizeSuggestions(suggestions);emit({type:'suggestions',suggestions:items});return {count:items.length,nextStep:'Finish your answer. Buttons appear only when the response finishes. Nothing has been executed.'};}),
  ...
  const prepareTurn=async context=>{
    const suggestionPolicy='When you recommend a useful next step or pause a multi-step mission, use suggest_next_steps for one to three short natural prompt buttons, such as "Draft the invitation" or "Check for replies". Include enough conversation context in each prompt to identify the intended action, without technical IDs or authorization wording unless needed to disambiguate. Do not force suggestions when the task is finished, suggest duplicate paid work, or offer payment approval shortcuts. Suggestions are allowed in read-only completion turns but execute nothing. Never say you will do a next step automatically when waiting for the user to choose it.';
