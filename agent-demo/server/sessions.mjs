...
export class Sessions {
  ...
  close(token) {const s=this.items.get(token);if(s?.budget)s.budget.revoked=true;for(const c of s?.a2aControllers?.values()||[])c.abort();s?.agent?.abort('Chat closed');this.items.delete(token);}
