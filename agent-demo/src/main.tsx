...
const textValue=(v:unknown):string=>v===null||v===undefined?'—':typeof v==='object'?JSON.stringify(v):String(v);
...
function Payment({op,request,onUpdate,onPaid}:{op:Operation;request:Function;onUpdate:Function;onPaid:Function}) {
  ...
  return <div className="payment-wrap">
    <div className="payment-card"><div className="payment-top"><span className="card-chip" aria-hidden="true">▦</span><span>Wikshi · x402</span><span>TESTNET</span></div><p className="card-caption">One small payment. A useful next step.</p><h3>{quote?(Number(quote.amount)/10**(currency==='HBAR'?8:6)).toLocaleString(undefined,{maximumFractionDigits:8}):'—'} <span>{currency}</span></h3><div className="currency-switch" aria-label="Payment currency">{['USDC','HBAR'].map(c=><button key={c} aria-pressed={currency===c} onClick={()=>setCurrency(c)} disabled={busy}>{c}</button>)}</div><div className="payment-bottom"><span>{op.service}</span><span>To {quote?.payTo||'—'}</span></div></div>
    ...
    <div className="payment-actions"><button className="primary" disabled={busy||expired} onClick={()=>pay()}>{expired?'Quote expired':busy?'Waiting for payment…':'Approve & sign in wallet ↗'}</button><button ref={skip} className="text-button" disabled={busy||expired} onClick={()=>{setError('');dialog.current?.showModal();}}>Skip — let Wikshi sponsor</button></div>
...
function App(){
  ...
  const bottom=useRef<HTMLDivElement>(null),pending=useRef(false),sessionToken=useRef('');
  ...
  return <div className="chat-shell"><header className="chat-header"><a href="https://wikshi.xyz" className="wordmark"><img src={art+'messenger-cutout.png'} alt=""/>Wikshi</a><span className="header-note">A little help. A lot of follow-through.</span><button className="session-button" onClick={()=>setSidebar(!sidebar)} aria-expanded={sidebar}>Your workspace <span aria-hidden="true">↗</span></button></header>
    <main className="chat-main"><section className="conversation" aria-label="Agent conversation">
      {!messages.length&&<div className="welcome"><div className="welcome-label"><span/> YOUR WIKSHI AGENT</div><h1>Who should we<br/><em>reach today?</em></h1><p>Find the right people. Start a conversation.<br/>I’ll help with the follow-through.</p><Bird/><div className="starters">{['Find voice AI startups in India','Find a venue for my next event','Help me follow up by email'].map(s=><button key={s} disabled={!token||busy} onClick={()=>send(s)}>{s}<span>↗</span></button>)}</div></div>}
