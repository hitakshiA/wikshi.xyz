...
import './cards.css';
import {Swap,Presence} from './motion';
...
export function ToolActivity({name}:{name:string}) {
  ...
  return <Swap identity={state}><div className={`tool-activity activity-${state}`} role="status"><Bird state={state}/><div><strong>{state==='search'?'Following the leads':state==='email'?'Preparing your correspondence':state==='call'?'Checking the conversation':state==='payment'?'Preparing your payment card':'Working on your mission'}</strong><span>{name||'Connecting the next steps'}<i className="stream-dot">●</i></span></div></div></Swap>;
}
...
export function EmailDrafts({batch,onPrepare,onDecision,busy,revisingId=''}:{batch:DraftBatch;onPrepare:(id:string)=>void;onDecision:(id:string,decision:string,feedback?:string)=>void;busy:boolean;revisingId?:string}) {
...
export function Payment({op,request,onUpdate,onPaid,onRefresh,batchOps,disabled=false}:{op:Operation;request:Function;onUpdate:Function;onPaid:Function;onRefresh?:()=>void|Promise<void>;batchOps?:Operation[];disabled?:boolean}) {
  ...
  return <div className="payment-wrap" aria-label={`${serviceLabel} payment`}>
    ...
    <dialog ref={dialog} className="sponsor-dialog" onCancel={e=>{if(busy)e.preventDefault();}} onClose={()=>skip.current?.focus()}><Bird state="payment"/><h2>{busy&&sponsoring?'Taking care of payment…':'We’ll pick this one up.'}</h2><p>{busy&&sponsoring?'Your sponsored payment is processing. This can take a moment.':`Approve ${serviceLabel.toLowerCase()} using sponsored testnet ${currency}.`}</p>{error&&<p role="alert">{error}</p>}<button autoFocus className="primary sponsor-confirm" aria-busy={busy&&sponsoring} disabled={locked||expired} onClick={()=>pay(true)}>{busy&&sponsoring&&<span className="payment-spinner" aria-hidden="true"/>}{busy&&sponsoring?'Processing payment…':'Approve sponsored payment'}</button><div className="sponsor-progress" role="status" aria-live="polite">{busy&&sponsoring?'Please keep this window open. Your request will update automatically.':''}</div><button className="text-button" disabled={busy} onClick={()=>dialog.current?.close()}>Back</button></dialog>
  </div>;
}
...
export function OperationCard({op,request,onUpdate,onCheck,onPaid,onEmail,onRefresh,onOpenResearch,busy}:{op:Operation;request:Function;onUpdate:Function;onCheck:Function;onPaid?:(id:string,operations:Operation[])=>void;onEmail:(id:string)=>void;onRefresh?:(id:string)=>void|Promise<void>;onOpenResearch?:(id:string)=>void;busy:boolean}) {
  ...
  return <Swap identity={waiting?'payment':'result'}>{renderCard()}</Swap>;
}
