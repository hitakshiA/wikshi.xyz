...
export function OperationCard({op,request,onUpdate,onCheck,onPaid,onEmail,onRefresh,onOpenResearch,busy}:{op:Operation;request:Function;onUpdate:Function;onCheck:Function;onPaid?:(id:string,operations:Operation[])=>void;onEmail:(id:string)=>void;onRefresh?:(id:string)=>void|Promise<void>;onOpenResearch?:(id:string)=>void;busy:boolean}) {
  ...
  useEffect(()=>{if(!pollable)return;let stopped=false,timer:ReturnType<typeof setTimeout>;const poll=async()=>{try{const next=await fetcher.current(`/operations/${op.id}`);if(!stopped){updater.current(next);setPollError('');if(!shouldPoll(next))return;}}catch{if(!stopped)setPollError('Connection interrupted. Checking again shortly.');}if(!stopped)timer=setTimeout(poll,calling?5000:2000);};timer=setTimeout(poll,1000);return()=>{stopped=true;clearTimeout(timer);};},[op.id,pollable,calling]);
  ...
  if(op.status==='cancelled')return <div className="cancelled-request" data-operation={op.id}><strong>Request cancelled</strong><span>{op.payment?.confirmed?'Payment details are in your workspace.':'No payment was made.'}</span></div>;
  if(research&&!waiting)return <article className="operation research-operation" data-operation={op.id}>
    {op.status==='completed'?<ResearchAttachment op={op} onOpen={()=>onOpenResearch?.(op.id)}/>:<div className="research-progress" role="status"><Bird state={active?'search':'thinking'}/><div><strong>{active?title:op.status==='failed'?'Research could not finish':'Research update'}</strong><span>{active?(/payment/.test(op.status)?'Confirming your payment.':op.status==='queued'?'Payment confirmed. Your research is queued.':'Following the leads. Results will appear here.'):(op.error?.code||op.status.replaceAll('_',' '))}</span>{pollError&&<small>{pollError}</small>}</div>{pollError&&<button className="text-button" disabled={checking} onClick={checkStatus}>Retry</button>}</div>}
    <Receipt op={op}/>
  </article>;
